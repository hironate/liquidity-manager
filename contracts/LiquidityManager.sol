...
contract LiquidityManager is IERC721Receiver, Ownable {
    ...
    function decreaseLiquidity(uint128 _liquidity, uint256 _tokenId) onlyOwner external returns (uint amount0, uint amount1) {

        require(nonfungiblePositionManager.isApprovedForAll(msg.sender, address(this)),'Position is not approved for Position Manager Contract');
    ...
    function collectAllFees( uint256 _tokenId) onlyOwner external returns (uint256 amount0, uint256 amount1) {
